...
const FavouritePlaces = ({navigation}: HomeNavigationProps<"FavouritePlaces">) => {
    ...
    storage.load({
        ...
        console.log('vafler', ret.marker)
        setCurrentMarker(ret);
    })
        .catch(err => {
                ...
                <ScrollView contentContainerStyle={{
                    ...
                    <Transitioning.View ref={list} transition={transition}>
                        <Box flexDirection='row'>
                            <Box marginRight='s'>
                                {places
                                    .filter((_, i) => i % 2 !== 0).map((place) => <Picture key={place.id}
                                                                                           place={place}
                                                                                           width={width}/>)}
                            </Box>
                            <Box>
                                {places.filter((_, i) => i % 2 === 0).map((place) => <Picture key={place.id}
                                                                                                   place={place}
                                                                                                   width={width}/>)}
                            </Box>
                        </Box>
